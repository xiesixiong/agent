...
class Agent:
        ...
        #第九个功能：让助手启动功能
            {
                ...
                "function": {
                    "name": "create_agents",
                    "description": "Create an agent with specific functions to specialize in completing a particular task.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "task_description": {
                                "type": "string",
                                "description": "Description of the task you want this agent to complete."
                            },
                            "agent_name": {
                                "type": "string",
                                "description": "The name of this agent should be named."
                            }
                        },
            ...
            # 第十个功能:得到子agent的答案
            {
                ...
                "function": {
                    "name": "call_agents",
                    "description": "When you need a specific function agent to help you complete youir task, you can use this function.",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "agent_name": {
                                "type": "string",
                                "description": "This name should refer to the designated agent to perform this task."
                            },
                            "specific_questions": {
                                "type": "string",
                                "description": "This specific_questions description  about what you need this agent to help you solve."
                            }
                        },
                        "required": [ "agent_name","specific_questions"],
                        "additionalProperties": False
                    },
                    "strict": True
                }
    ...
    def create_agents(agent,task_description,agent_name):
        #先创建agent的数据集
        agent_data = []
        ...
        for task in own_task:
            ...
            agent_data.append(completion.choices[0].message.content)

        with open(f'{agent_name}dataset.txt', 'w',encoding="utf-8") as file:
            for item in agent_data:
                file.write(item + '\n')

        now_agent = Agent(api_key=agent.api_key,goal_prompt_path="son_goal_prompt.md",own_task_pth=f'{agent_name}dataset.txt',father_or_son=0,name=agent_name)

        agent_groups[agent_name] = {"object": now_agent, "description": f"This son_agent is used to solve {task_description} "}
        thread = threading.Thread(target=now_agent.evolve)
        thread.start()
        ...
        with open(f"{agent.name}的日志", 'a', encoding="utf-8") as f:
            ...
            f.write(f"  - 正在调用函数: create_agents\n")
            f.write(f"  - 任务描述: {task_description}\n")
            f.write(f"  - 子Agent名字: {agent_name}\n")
            f.write(f"  - 当前这个agent数据集已创建，且Agent已开始运行!\n")
            f.write("\n\n")  # 添加空行分隔日志条目

        print(( f"当前agent是{agent.name}正在调用函数create_son_agents，创建针对于{task_description}的数据集，并命名该agent名字为{agent_name}，数据集已创建好！为了解决{task_description}的agent已开始运行！\n\n"))
        return "create_agents is ok!"




    # 把对应的任务输入给对应的子agent，得到结果
    def call_agents(agent,agent_name,specific_questions):

        now_agent = agent_groups[agent_name]["object"]
        answer = now_agent.best_solver(specific_questions) # todo: son_agent.archived_solver; son_agent.archived_score
        ...
        with open(f"{agent.name}的日志", 'a', encoding="utf-8") as f:
            ...
            f.write(f"  - 被调用的子Agent: {agent_name}\n")
        ...
        print((f"当前agent是{agent.name},他正在调用函数  call_son_agents ，call的是子agent{agent_name}，当前子agent处理的任务是{specific_questions}，返回的结果是{answer} \n\n"))
        ...
        for tool_call in actions['tool_calls']:
            ...
            try:
                ...
                if tool_call['function']['name'] == "action_display_analysis":
                    ...
                    result = agent.create_agents(**arguments)


                elif tool_call['function']['name'] == "call_agents":
                    result = agent.call_agents(**arguments)
